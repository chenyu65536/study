...
public class Q71 {
    ...
    public String simplifyPath(String path) {
        ...
        for (int i = 0; i < directory.length; i++) {
            ...
            if (directory[i].equals(".")) {//本级目录
                continue;
            }
            if (directory[i].equals("..")) {//上级目录
                if (!stack.isEmpty()) {
                    String popStr = stack.pop();
                    sb.delete(sb.length() - popStr.length() - 1, sb.length());
                }
                continue;
            }
            stack.push(directory[i]);
            sb.append("/").append(directory[i]);
        }
        if (stack.isEmpty()) {
            return "/";
        }
    ...
    public static void main(String[] args) {
        ...
        Q71 q71 = new Q71();
        String rs = q71.simplifyPath(path);
        System.out.printf("");
